/*  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    Last Edit (NT): 2026-08-11 09:53:08
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
$().ready(function () {
    ...
    //	Added by NEIL
    $('html').css({'--dataFontSize': cmxConfig.Geometry.DataFontSize + '%','--dataFontWeight': cmxConfig.Geometry.DataFontWeight});
    $('html').css('--ledOff', cmxConfig.LEDs.OffColour );
    ...
    if(! cmxConfig.ShowAlarms ) {
        $('#AlarmsPanel').addClass('ow-hide');
        $('#Alarms').text('{{SHOW}} {{ALARM}}');
    } else {
        $('#AlarmsPanel').removeClass('ow-hide');
            ...
            if (key == 'Alarms') {
                data[key].forEach(function (alarm) {
                    ...
                    if (alarm.triggered && alarmState[alarm.id] == false) {
                        ...
                        $(alarmtag).removeClass('ow-led-on')/*.addClass('ax-led-off')*/;
    ...
    function clearAlarm(id) {
        ...
        .done(function(result) {
            if (result == "Cleared") {
                ...
                alarmState[id] = false;
                let alarmtag = '#' + id;
                $(alarmtag).removeClass('ow-led-on');
                $(alarmtag).prop("onclick", null).off("click");
            } else {
                log(id + ' error: ' + result);
            }
    ...
    .done(function (result) {
        ...
        result.forEach(function (alarm) {
            ...
            if(!alarm.Id.search('User')) {
                $('#alarms').append('<div class="ledGrid"><div class="ow-led ' + cmxConfig.LEDs.User + '" id="' + alarmTag + '"></div><p>' + alarm.Name + '</p></div>');
            } else {
                $('#alarms').append('<div class="ledGrid"><div class="ow-led ' + cmxConfig.LEDs.Default + '" id="' + alarmTag + '"></div>' + alarm.Name + '</div>');
            }
        ...
        switch( result.stationId){
            ...
            case '11':
                ...
                if( cmxConfig.ShowDavis ) {
                    $('#DavisPanel').removeClass('ow-hide');
                    $('#Davis').text('{{HIDE}} Davis');
                } else {
                    $('#DavisPanel').addClass('ow-hide');
                    $('#Davis').text('{{SHOW}} Davis');
                }
                break;
            default:
                $('#Davis').addClass('ow-hide');
                $('#DavisPanel').addClass('ow-hide')
        }
    })
}
    ...
    .done(function(data) {
        ...
        for (var i = 0; i < 8; i++) {
            if (batteries[i].includes('NA')) {
                $('#DavisXmt' + i).addClass('ow-hide');
                $('#DavisPct' + i).addClass('ow-hide');
                $('#DavisRssi' + i).addClass('w3-hide');
            } else {
                $('#DavisXmt' + i).removeClass('ow-hide');
                $('#DavisPct' + i).removeClass('ow-hide');
                $('#DavisRssi' + i).removeClass('ow-hide');
