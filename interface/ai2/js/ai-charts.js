...
$().ready(() => {
//    });
    $('.ow-btnBar').children().on('click', function(){
        $('.ow-btnBar').children().removeClass('ow-disabled'); //  Enable all buttons
        $('#' + this.id).addClass('ow-disabled');               //  Disable this button
        cmxSession.Charts.Trends = this.id;                     //  Store selected chart
    ...
    .then((results) => {
        ...
        temperatureScale = {
            ...
            grid: {
                ...
                //color: (line) => (line.tick.value === 4 ? 'red' : '')
            },
            ticks: {
                color: (context) => {
        ...
        var currentChart = cmxSession.Charts.Trends;
        if( currentChart =='' ) {
            currentChart = 'temp';
        }
        $('.ow-btnBar').children().removeClass('ow-disabled');
        $('#' + currentChart).addClass('ow-disabled');
        doSelect( currentChart);
    });

    doSelect = (sel) => {
        CmxChartJsHelpers.ShowLoading();

        console.log("doSelect recieved: " + sel );
        switch (sel) {
            case 'temp':
                doTemp();
                break;
