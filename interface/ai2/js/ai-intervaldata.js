/*  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    Last Edit (NT):  2026-08-11 09:56:28
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
$().ready(function () {
    ...
    Promise.all([visibility, names])
        .then(function (results) {
            ...
            // If you prefer the last two panels to be smaller add the class 'ow-fixed' below.
            cards = $('<div>', { id: 'panels', class: 'ow-flex col4 ow-fixed', style: 'align-items:start' });
            ...
            if (tempBoxes.children().length > 0) {
                let tempBlock = $('<div>', { class: 'my-unit' })
                    ...
                    .append(tempBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:0' }).append(tempBlock));
            }
            ...
            if (humBoxes.children().length > 0) {
                let humBlock = $('<div>', { class: 'my-unit' })
                    ...
                    .append(humBoxes);
                cards.append($('<div>', {class: 'ow-panel ow-theme8', style:'order:0;'}).append(humBlock));
            }
            ...
            cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:0;' }).append(pressBlock));
            ...
            cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:1;'}).append(windBlock));
            ...
            cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:0'}).append(rainBlock));
            ...
            if (solarBoxes.children().length > 0) {
                let solarBlock = $('<div>', { class: 'my-unit' })
                    ...
                    .append(solarBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:2'}).append(solarBlock));
            }
            ...
            if (extraTempBoxes.children().length > 0) {
                let extraTempBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{EXTRA_TEMPERATURE}}</h2>' }))
                    .append(extraTempBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(extraTempBlock));
            }
            ...
            if (extraHumBoxes.children().length > 0) {
                let extraHumBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{EXTRA_HUMIDITY}}</h2>' }))
                    .append(extraHumBoxes);
                cards.append($('<div>', {class: 'ow-panel ow-theme8', style:'order:3;'}).append(extraHumBlock));
            }
            ...
            if (extraDewBoxes.children().length > 0) {
                let extraDewBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{EXTRA_DEW_POINT}}</h2>' }))
                    .append(extraDewBoxes);
                cards.append($('<div>', {class: 'ow-panel ow-theme8', style:'order:3;'}).append(extraDewBlock));
            }
            ...
            if (soilTempBoxes.children().length > 0) {
                let soilTempBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{SOIL_TEMPERATURE}}</h2>' }))
                    .append(soilTempBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(soilTempBlock));
            }
            ...
            if (soilMoistBoxes.children().length > 0) {
                let soilMoistBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{SOIL_MOISTURE}}</h2>' }))
                    .append(soilMoistBoxes);
                cards.append($('<div>', {class: 'ow-panel ow-theme8', style:'order:3;'}).append(soilMoistBlock));
            }
            ...
            if (leafWetBoxes.children().length > 0) {
                let leafwetBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{LEAF_WETNESS}}</h2>' }))
                    .append(leafWetBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(leafwetBlock));
            }
            ...
            if (userTempBoxes.children().length > 0) {
                let usertempBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{USER_TEMPERATURE}}</h2>' }))
                    .append(userTempBoxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(usertempBlock));
            }
            ...
            if (aqSensors.children().length > 0) {
                let aqBlock = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>{{AIR_QUALITY}}</h2>' }))
                    .append(aqSensors);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(aqBlock));
            }
            ...
            if (co2Boxes.children().length > 0) {
                let co2Block = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>CO₂</h2>' }))
                    .append(co2Boxes);
                cards.append($('<div>', { class: 'ow-panel ow-theme8', style:'order:3;'}).append(co2Block));
            }
            // laser distance
            let laserBoxes = $('<div>');
            for (var i = 0; i < 8; i++) {
            ...
            if (laserBoxes.children().length > 0) {
                let laserBox = $('<div>', { class: 'my-unit' })
                    .append($('<div>', { class: 'my-title', html: '<h2>Laser Distance</h2>' }))
                    .append(laserBoxes);
                cards.append($('<div>', {class:'ow-panel ow-theme8', style: 'order:3'}).append(laserBox));
            }
            ...
            if (snowBoxes.children().length > 0) {
                let snowBox = $('<div>', { class:'my-unit'})
                    .append($('<div>', { class: 'ow-titleBar', html: '<h2>Snow 24h</h2>' }))
                    .append(snowBoxes);
                cards.append($('<div>', {class: 'ow-panel ow-theme8', style:'order:3'}).append(snowBox));
            }
...
function createDataPage(result) {
	...
	html += '<h3>{{INTERVAL_DATA_VIEWER}}</h3></div>';
