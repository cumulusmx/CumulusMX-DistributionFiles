/* ======================================================================================================================================================
	This is one of the JavaScript files that is used in CumulusMX admin interface for editing and deleting lines in the log files 
	(see https://github.com/cumulusmx/CumulusMX-DistributionFiles/blob/master/interface/datalogs.html)
   ======================================================================================================================================================
   Created by sfws - May 2020 for incorporation in future Cumulus MX release (probably build 3084)
   ======================================================================================================================================================
This script by sfws is a slightly modified version of
	"https://github.com/KasperOlesen/DataTable-AltEditor/blob/master/src/dataTables.altEditor.free.js"
	
The parts of that script dealing with "Add a new record" have been removed, as they are not needed in this context.
	
There are 5 edits within
	--------------------------------------------------------------
			$.extend(
						altEditor.prototype
	--------------------------------------------------------------
	Specifically, these have been amended:
	1)	_setup: function() - edited "button: this.language.edit.button || 'Save edit'" (i.e. given new default name)
	2)	_openEditModal: function() - added a new function called within "for (var j in columnDefs)" loop to permit customisation of the output for individual fields
								(this allows me to replace fields read from the dataTables line with the derived values I have calculated amongst other benefits)
	3)	createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName)
											- added the extra "label" for those fields where derived values output, this is where I put other customised text
	4)	createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName)
											- amended  $(selector).find('.modal-footer') to add footer text (I don't need this, but it is easier this way than the technique used in the other log editors)
	5)	completeColumnDefs: function () - added default value of false for extra parameter "derived:" (I could have adopted an existing parameter, but this is more meaningful)
	--------------------------------------------------------------
Here follows the full description as included in that original file:
 *
	* @summary altEditor
	...
	*
	* This source file is distributed in the hope that it will be useful, but  WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
...
(function ($, window, document, undefined) {
    ...
    /**
     * altEditor provides modal editing of records for DataTables software
     *
     * @class altEditor
     * @constructor
     * @param {object}
    ...
    $.extend(
		...
		/* EDIT */
                ...
                this.language.edit = { title: this.language.edit.title || 'Edit record',
                                       button: this.language.edit.button || 'Save edit' };
		/* DELETE */ 
		...
		/* DEFINE A MODA (a pop up dialog) */
                var modal =
			   '<article class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog">' +
                    '<section class="modal-dialog">' +
					'<main class="modal-content">' +
						'<div class="modal-header">' +
							'<h4 style="padding-top: 1rem;padding-left: 1rem;" class="modal-title"></h4>' +
							'<button style="margin: initial;" type="button" class="text-info bg-success close" data-dismiss="modal" aria-label="' + this.language.modalClose + '">' +
           ...
           // add modal
                $('body').append(modal); /* adds modal just before </body> on web page */
			 
          // add Edit this row Button to main datatables object, with action to open the editing modal
                if (dt.button('edit:name')) {
                    dt.button('edit:name').action(function (e, dt, node, config) {
                        that._openEditModal();
		/* give modal a unique instance name */
          ...
          // add Delete this row Button to main datatables object, with action to open the delete confirming modal
...
/*	*
		* Add the values to the form in the Edit Modal used to edit selected row
		*
          * @private
*/
	...
	/* new code specific to Cumulus MX inserted here */
				    insertValues(columnDefs[j].name, columnDefs[j].title, selector, jquerySelector, this._quoteattr(selectedValue));
				/* replaces  
				    $(selector).find(jquerySelector).val(quotedValue);  // now incorporated in function called above so can be optional
				*/
	/* new code specific to Cumulus MX ends here */  
                        $(selector).find(jquerySelector).trigger("change"); // required by select2
                    }	// end if
                }  // end loop              
                $(selector + ' input[3]').focus();
                $(selector).trigger("alteditor:some_dialog_opened").trigger("alteditor:edit_dialog_opened");
            },

	 /* Handles provided by standard altEditor from the editRowDate modal */
            /**
             * Callback for "Save edit" button  (previously called "Edit" button)
             */
		_editRowData: function () {
			...
			/* next line added */
                ...
                // Getting the contents of input fields from the edit-modal
                $(`form[name="altEditor-edit-form-${this.random_id}"] *`).filter(':input[type!="file"]').each(function (i) {
                    rowDataArray[$(this).attr('id')] = $(this).val();
                });
                //Getting the textArea fields from the modal
 ...
 /*	*
		* Add the values to the form in the Delete Modal used to confirm deletion of selected row 
		*
             * @private
     */
            _openDeleteModal: function () {
                ...
                for (var j in columnDefs) {
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        data += "<section style='margin-left: initial;margin-right: initial;' class='form-group row'><label for='"
            ...
            */
            completeColumnDefs: function () {
                ...
                for (var i in dt.context[0].aoColumns) {
				    ...
				    /* Next item added */
				    derived: (obj.derived ? obj.derived : false),
				    /* end of edit */
...
/*	*
     * Create both Edit and Add modals (dialogs) as a HTML form with label, input, and select, fields
	*
            ...
            */
            createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
                formName = [formName, this.random_id].join('-');  /* gives form a unique name */
                var data = "";
                for (var j in columnDefs) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        data += "<section class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:4px;'>";
                        data += "<label for='" + columnDefs[j].name + "'>" + columnDefs[j].title + ":</label></section>";
                        data += "<article class='col-sm-8 col-md-8 col-lg-8'>";
                        ...
                        else if (columnDefs[j].type.indexOf("select") >= 0)
				    {
                        ...
                        // Adding text-inputs and error labels, but also new HTML5 types (email, color, ...)
                            ...
                            data += "<input type='" + this._quoteattr(columnDefs[j].type)
                                ...
                                + (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'")
                                + " style='overflow:hidden'  class='form-control  form-control-sm' value=''>";
		/* New functionality for MX starts here  - derived fields can have extra text added such as derived values in extra label field to hint at how might update */
						  data += (columnDefs[j].derived ? "<label id='" + j  + "a'></label>" : '');
		/* end of added new functionality */
                        }
                        data += "<label id='" + this._quoteattr(columnDefs[j].name) + "label"
                                + "' class='errorLabel'></label>";  // space for error message after every field
                        data += "</article><div style='clear:both;'></div>";
                    }
                }
                var selector = this.modal_selector;
                $(selector).on('show.bs.modal', function () {
	...
	// custom onchange triggers
                    if (columnDefs[j].editorOnChange) {
                        var f = columnDefs[j].editorOnChange; /* in commit of 1 June 2020 luca-vercelli  deleted this line and inserted 2 others
				    but this stops my onchange functionality working, so sticking with code as it was when copied for MX */
     ...
     */
            ...
            /**
             * Default callback for editing: mock web service, always success.
             */
            ...
            /**
             * Default callback for deletion: mock web service, always success.
             */
...
/* ======================================================================================
		***********	end of modified dataTables.altEditor.free	 **************
 ========================================================================================= */