...
$(document).ready(() => {
    $('.btn').change(() => {
        const myRadio = $('input[name=options]');
        const checkedValue = myRadio.filter(':checked').val();
    ...
    .then(function (results) {
        ...
        const value = parent.location.hash.replace('#', '');
        doGraph(value);
        ...
        idxs.forEach((idx) => {
            if (idx in resp) {
                ...
                dataSets.push({
                    ...
                    tooltip: {
                        callbacks: {
                            label: item => ` ${item.dataset.label} ${item.parsed.y?.toFixedMX(config.temp.decimals) ?? '—'} ${suffix}`
                        }
                    },
                    hidden: hidden[idx]
                });
...
const doWind = () => {
    ...
    .done((resp) => {
        ...
        const dataSets = [{
            ...
            tooltip: {
                callbacks: {
                    label: item => ` ${item.dataset.label} ${item.parsed.y?.toFixedMX(1) ?? '—'} ${config.wind.rununits}`
                }
            },
            hidden: true
        }];
...
const doRain = () => {
    ...
    .done((resp) => {
        ...
        const dataSets = [{
            ...
            tooltip: {
                callbacks: {
                    label: item => ` ${item.dataset.label} ${item.parsed.y?.toFixedMX(config.rain.decimals) ?? '—'} ${config.rain.units}/{{HOUR_SHORT_LC}}`
                }
            },
            order: 0,
            hidden: true
        ...
        idxs.forEach((idx) => {
            if (idx in resp) {
                ...
                dataSets.push({
                    ...
                    tooltip: {
                        callbacks: {
                            label: item => ` ${item.dataset.label} ${item.parsed.y?.toFixedMX(tooltips[idx].valueDecimals) ?? '—'} ${tooltips[idx].valueSuffix}`
                        }
                    },
                    order: indexes[idx]
                });
            ...
            if (idx in resp) {
                Object.keys(resp[idx]).forEach(yr => {
                    dataSets.push({
                        ...
                        tooltip: {
                            callbacks: {
                                label: item => ` ${item.dataset.label} ${item.parsed.y?.toFixedMX(1) ?? '—'}`
                            }
                        },
                        hidden: true
                    });
